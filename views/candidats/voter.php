...
<script>
    (function() {
        ...
        const STATUS_URL = <?= json_encode($base_url); ?> + '/api/vote/status';
...
<script>
    function choixVotePoste(postId, posteName, candidateId, candidateNom) {
        const fetch_url_poste = <?= json_encode($base_url); ?> + '/choix/vote/poste'
        fetch(fetch_url_poste, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
            body: JSON.stringify({
                candidateName: candidateNom,
                posteName: posteName,
...
<?php 
$nbPostesActif = (new CandidatRepository())->getNbPostesActif();
?>
<script>
    //let memoire = [];
    const memoire = new Map();
    let nombrePostesActifs = <?= json_encode((int)$nbPostesActif); ?>

    console.log("Nombre de postes actifs :", nombrePostesActifs);
        ...
        group => {
            ...
            cards.forEach(card => {
                card.addEventListener("click", () => {
                    ...
                    choixVotePoste(postId, posteName, candidateId, candidateNom);

                    if (memoire.size == nombrePostesActifs) {
